-- Safe Equipment Table Setup Script
-- This script handles existing tables and policies safely

-- First, check if equipment table exists and show its structure
SELECT 
    column_name, 
    data_type, 
    is_nullable,
...
-- Check current data in equipment table
SELECT COUNT(*) as current_equipment_count FROM equipment;
...
-- Insert sample equipment data (will only insert if not already present)
INSERT INTO equipment (name, brand, model, category, studio_id, room, description, image_url) 
SELECT * FROM (VALUES
...
) AS new_equipment(name, brand, model, category, studio_id, room, description, image_url)
WHERE NOT EXISTS (
    SELECT 1 FROM equipment WHERE equipment.name = new_equipment.name
);

-- Final verification
SELECT 'Equipment setup completed!' as status;
SELECT COUNT(*) as final_equipment_count FROM equipment;
SELECT name, brand, category FROM equipment ORDER BY name;
